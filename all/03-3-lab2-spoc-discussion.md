...
可以运行的指令集不同，可访问的数据范围不同，可能的行为不同

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

RPL：表示当前要访问的数据段对应的特权级
CPL：表示当前代码的特权级
DPL：访问目标的特权级
访问门时：CPL<=DPL（门），CPL>=DPL（段）
访问段时：CPL<=DPL（段），RPL<=DPL（段）


4. 写一个示例程序，完成4个特权级间的函数调用和数据访问时特权级控制的作用。

- 访问数据时，检查CPL/DPL和RPL之间的关系
- 转移程序控制权时，由低等级代码转移给高等级代码
...
当低特权级代码调用高特权级代码，或发生中断、异常时。

3. int指令在ring0和ring3的执行行为有什么不同？

压栈内容是不同的，ring0下不需要压入SS和ESP。也不需要进行栈切换。
...
TSS:记录任务信息、用于任务管理
Task Register：缓存TSS描述符内容，记录TSS段选择子，段基址和segment limit。
...
先从逻辑地址得到段选择子，然后查找GDT得到段描述符，将段基址与offset相加后得到线性地址空间中的地址。若未启动页机制，则需要抓换的物理地址就是该线性地址。若启动了页机制，则通过线性地址得到页目录索引，与页目录基址相加得到获得页目录项，得到页表基址，与表内偏移相加后得到页表项，得到页基址，与offset相加得到所需要计算的物理地址。

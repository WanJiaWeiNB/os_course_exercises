...
1. 寄存器、高速缓存、内存、外存的访问特征？
   ...
   从左到右存取速率递减，访问频率递减。寄存器和高速缓存不需要软件参与存储管理，内存和外存需要软件参与。存储器的位置也不同。
...
1. 什么是时间局部性、空间局部性和分支局部性？
   ...
   时间局部性：存储器倾向于在不久之后访问同一块存储位置。

    空间局部性：存储器倾向于在不久之后访问之前访问的存储位置的相邻位置。

   分支局部性：分支跳转倾向于与之前的跳转结果一样。
